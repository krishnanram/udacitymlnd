...
class Environment(object):
    ...
    def __init__(self, dummyAgent,start,destination):
    ...
    def reset(self):
        ...
        if self.start == None or self.destination == None :
            self.start = random.choice(self.intersections.keys())
            self.destination = random.choice(self.intersections.keys())

        # Ensure starting location and destination are not too close
        while self.compute_dist(self.start, self.destination) < 4:
            start = random.choice(self.intersections.keys())
            destination = random.choice(self.intersections.keys())

        start_heading = random.choice(self.valid_headings)
        deadline = self.compute_dist(self.start, self.destination) * 5
        print "Environment.reset(): Trial set up with start = {}, destination = {}, deadline = {}".format(self.start, self.destination, deadline)

        # Initialize agent(s)
        for agent in self.agent_states.iterkeys():
            self.agent_states[agent] = {
                'location': self.start if agent is self.primary_agent else random.choice(self.intersections.keys()),
                'heading': start_heading if agent is self.primary_agent else random.choice(self.valid_headings),
                'destination': self.destination if agent is self.primary_agent else None,
                'deadline': deadline if agent is self.primary_agent else None}
            agent.reset(destination=(self.destination if agent is self.primary_agent else None))
    ...
    def step(self):
        ...
        if self.primary_agent is not None:
            if self.enforce_deadline and self.agent_states[self.primary_agent]['deadline'] <= 0:
                self.done = True
                stat = {
                    "deadline": self.agent_states[self.primary_agent]['deadline'],
                    'rewards': agent.getTotalRewards(),
                    'reached_dest': 'no',
                    'time_taken'  : ''
                }
                self.runTimeStat.setTotalRewards(stat)
                print "DONE:Environment.reset(): Primary agent could not reach destination within deadline!"
    ...
    def act(self, agent, action):
        ...
        if agent is self.primary_agent:
            if state['location'] == state['destination']:
                ...
                self.runTimeStat.setTotalRewards(stat)
                print "DONE:****************** SUCCESS ************ Primary agent has reached destination! in deadline:",state['deadline'], " Total Reward:", agent.getTotalRewards()
                print
